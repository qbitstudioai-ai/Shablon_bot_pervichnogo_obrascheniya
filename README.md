...
Подготовлена документационная основа v0.3. 23 сентября 2026 года Павел разрешил перейти к реализации шаблона. Текущий режим — **подготовка реализации**: PRE-01, DB-00, DB-01 и DB-02 завершены; DB-03 начат: DB-03A и DB-03B завершены; весь DB-03C (C1…C4) успешно применён и проверен в `qbit_bot_pervichnogo_obrascheniya`; DB-03C и DB-03D полностью применены и проверены в `qbit_bot_pervichnogo_obrascheniya`; D2 подтвердил atomic Take/Return/manual outgoing/private alert и controlled C4 manager-source upgrade. DB-03 полностью завершён и проверен в `qbit_bot_pervichnogo_obrascheniya`, включая DB-03V crossing `bot v_rabote → Take → late confirmed`: внешний факт сохраняется, stale effects/wait/t0/reminders не восстанавливаются. Перед PRE-02 выполняется DB-SCHEMA-01: test schema qBit переименовывается в `qbit_bot_pervichnogo_obrascheniya` с проверкой функций и прав. v0.1 остановился на database privilege, v0.2 — на чтении TEMP snapshot после SET ROLE; исправленный v0.3 временно выдаёт owner-role database CREATE только на rename/replace, затем до assertions делает RESET ROLE→postgres и отзывает CREATE; snapshots читаются без дополнительных runtime grants. После успешного server-check можно перейти в новую сессию к PRE-02/n8n. PRE-02 остаётся в работе. DB-02 успешно применён 24 сентября 2026 года в self-hosted Supabase: 13 таблиц `qbit_bot_pervichnogo_obrascheniya`, связи/индексы/COMMENT и защита от дубликата сообщения проверены; production не затронут. 24 сентября Павел импортировал и активировал draft клиентского CORE и служебного операторского Telegram; сквозные runtime-тесты, PostgreSQL-функции, Credentials/API и локальный STT ещё не проверены. Production и рабочий трафик не переключаются без отдельного явного разрешения.
