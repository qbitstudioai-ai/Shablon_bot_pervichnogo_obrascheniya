...
Текущая активная задача остаётся **PRE-02A**. Её можно закрыть независимо от WF-02 через изолированный обезличенный тест OpenRouter из self-hosted n8n с test Credential `QBIT_TEST_LLM`. Telegram/RT-01 до WF-02 не начинать. Для этого в репозитории подготовлен `workflows/PRE-02A_openrouter_smoke_test.json`; файл создан, но ещё не импортирован/не выполнен на сервере. Для отдельной географической диагностики также подготовлен `workflows/PRE-02A_openrouter_amsterdam_n8n_2.20.9.json`, совместимый с n8n 2.20.9 и использующий штатный `OpenRouter Chat Model`; он ещё не импортирован и не запускался. Если тест в Амстердаме успешен, это подтверждает работоспособность интеграции вне российского runtime, но не закрывает вопрос доступности OpenRouter с российского сервера.
