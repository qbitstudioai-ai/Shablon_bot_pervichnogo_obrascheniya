...
| `zabrat_ishodyashchee_deystvie` | bot | DB-03C4 + DB-03D2 upgrade, signature unchanged `jsonb`; `SKIP LOCKED` + lease/fencing; bot/system требуют bot-owner/block/initiative checks; `istochnik=menedzher` разрешён только для exact human owner/current manager/version из trusted action payload; stale action отменяется; `neizvestno` не claimится |
| `zafiksirovat_rezultat_ishodyashchego` | bot | DB-03C4 + DB-03D2 upgrade, signature unchanged `jsonb`; fenced result; confirmed/unknown/error fact сохраняется при in-flight state change; bot effects только при current bot state, manager retry/plain-confirm only при current human owner/current manager/version; confirmed manager message не создаёт `otvet_bota` mirror; bot t0/reminder semantics unchanged |
...
| `zabrat_dialog_operatorom` | sluzhebnyy | DB-03D2 signature `jsonb`; registered `callback_take` + allowed active manager + expected version; atomic `bot→chelovek` first-commit-wins; increment version/generation, clear wait/t0; cancel reminders/internal jobs and only not-yet-claimed bot/system actions; in-flight external attempts remain for factual result persistence; writes dialog+mirror events |
| `vernut_dialog_botu` | sluzhebnyy | DB-03D2 signature `jsonb`; registered `callback_return`, current active manager with return permission + expected version; `chelovek→bot`, manager=NULL, wait/t0=false, version/generation++; operator events only, **no client outgoing action** |
| `sozdat_ruchnoe_ishodyashchee` | sluzhebnyy | DB-03D2 signature `jsonb`; registered `operator_message`, confirmed service chat/thread, manager user id, text + expected version; resolves topic→dialog, verifies human owner/current manager, derives client channel only from DB, creates `soobshcheniya` author=manager + ordinary `istochnik=menedzher` outgoing action; idempotent by service event |
| `sozdat_lichnoe_uvedomlenie` | bot | DB-03D2 signature `jsonb`; stable key + manager UUID + dialog + reason/text; only active manager with confirmed private chat + notification setting; input does not accept target chat_id; creates mirror/private-notification event, D1 claim resolves trusted chat |
