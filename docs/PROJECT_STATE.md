...
Для PRE-02A нужен отдельный test Credential `QBIT_TEST_LLM` с ограниченным бюджетом и один обезличенный пробный вызов из n8n. Секрет не передаётся в чат и не сохраняется в workflow/GitHub. Runtime-вызов ещё не выполнен. Для воспроизводимой проверки подготовлен изолированный неактивный workflow `workflows/PRE-02A_openrouter_smoke_test.json`: Manual Trigger → один `POST /api/v1/chat/completions` → проверка model/structured output. Он не подключён к Telegram, Supabase или основному pipeline и не содержит Credential/секрет. Дополнительно для диагностического запуска на отдельном сервере Павла в Амстердаме с n8n 2.20.9 подготовлен `workflows/PRE-02A_openrouter_amsterdam_n8n_2.20.9.json`: Manual Trigger → Basic LLM Chain v1.9 → штатный OpenRouter Chat Model v1. Файл не содержит Credential/секретов/instanceId и пока только создан; импорт и runtime-проверка на сервере в Амстердаме ещё не выполнялись. Успех на Амстердаме подтвердит совместимость n8n/OpenRouter и модели, но сам по себе не докажет доступность OpenRouter с целевого российского runtime.
...
26 сентября 2026 года Павел сообщил результат географической проверки: OpenRouter Credential на российском n8n продолжал возвращать `403 Forbidden`, а на отдельном сервере в Амстердаме с n8n 2.20.9 подключение прошло без проблемы. Принято архитектурное решение: Supabase, CORE, очереди, PII, каналы и постоянные данные остаются на российском сервере; Amsterdam n8n используется как минимальный AI relay для OpenRouter. Relay не получает Supabase/PostgreSQL Credentials. Для RAG через relay должны выполняться и document embeddings при индексации, и query embeddings при каждом semantic-поиске одним model/dimension profile; поиск pgvector остаётся в России. Если ответы клиента генерирует OpenRouter, LLM-вызов также идёт через relay. PRE-02A ещё не закрыт: нужно сохранить доказательство фактического LLM-вызова, а не только успешного подключения Credential.
