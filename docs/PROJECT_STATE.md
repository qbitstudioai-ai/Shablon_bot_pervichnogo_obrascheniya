...
Завершена задача DOC-03. Первая эталонная установка создаётся для компании qBit; безопасный технический код — `qbit`. В PRE-01 подтверждены: n8n 2.41.0; PostgreSQL 17.6 (x86_64 Linux); n8n и Supabase открываются через веб-интерфейсы; сервер администрируется через терминал. Версия pgvector, состав/версии контейнеров Supabase, Auth, подключения и фактические настройки ещё не подтверждены. Instance ID n8n в публичную документацию не сохраняется. SQL, JSON workflow и код в этой сессии пока не создавались; настройки n8n, Supabase, Telegram и production не менялись.
...
PRE-01 начат 23 сентября 2026 года. Подтверждены n8n 2.41.0 и PostgreSQL 17.6; управление сервером выполняется через терминал. Следующий шаг — только чтением определить способ развёртывания и версии контейнеров Supabase, Auth и pgvector, затем проверить ограничения Telegram. Workflow в PRE-01 не проектируется. Перед началом последующего проектирования n8n необходимо отдельно запросить у Павла обещанный эталонный образец и изучить его.
