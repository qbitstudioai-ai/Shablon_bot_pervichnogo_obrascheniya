...
PRE-01 начат 23 сентября 2026 года. Подтверждены версии Docker, n8n, Supabase/PostgreSQL, Auth, Supavisor и основных контейнеров. Supabase развёрнут через Docker; n8n использует отдельный контейнер PostgreSQL. pgvector 0.8.2 доступен, но пока не активирован в базе. Следующий шаг — только чтением проверить сетевой путь n8n → Supabase PostgreSQL/Supavisor, опубликованные порты и затем ограничения Telegram. Workflow в PRE-01 не проектируется. Перед началом последующего проектирования n8n необходимо отдельно запросить у Павла обещанный эталонный образец и изучить его.
