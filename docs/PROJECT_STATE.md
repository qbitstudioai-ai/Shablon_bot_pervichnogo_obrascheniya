...
PRE-01 начат 23 сентября 2026 года. Подтверждены версии Docker, n8n, Supabase/PostgreSQL, Auth, Supavisor и основных контейнеров. Supabase развёрнут через Docker; n8n использует отдельный контейнер PostgreSQL. pgvector 0.8.2 доступен, но пока не активирован в базе. Павел разрешил подготовить Supabase как рабочую платформу для шаблона. Следующий шаг — включить штатный `vector` extension в schema `extensions`, проверить тип `vector` и оператор расстояния; после подтверждения продолжить проверкой сетевого пути n8n → Supabase PostgreSQL/Supavisor и ограничений Telegram. Workflow в PRE-01 не проектируется. Перед началом последующего проектирования n8n необходимо отдельно запросить у Павла обещанный эталонный образец и изучить его.
