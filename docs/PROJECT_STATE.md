...
PRE-01 начат 23 сентября 2026 года. Подтверждены версии Docker, n8n, Supabase/PostgreSQL, Auth, Supavisor и основных контейнеров. Supabase развёрнут через Docker; n8n использует отдельный контейнер PostgreSQL. pgvector 0.8.2 доступен, но пока не активирован в базе. Павел разрешил подготовить Supabase как рабочую платформу для шаблона. `vector` уже включён и проверен. Перед включением дополнительных расширений проводится инвентаризация доступных/установленных модулей и фиксируется минимальный базовый профиль: обязательные расширения включаются сразу, специализированные — только под конкретные приложения. После этого продолжается проверка сетевого пути n8n → Supabase PostgreSQL/Supavisor и ограничений Telegram. Workflow в PRE-01 не проектируется. Перед началом последующего проектирования n8n необходимо отдельно запросить у Павла обещанный эталонный образец и изучить его.
