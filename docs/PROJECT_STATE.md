...
Дополнительная проверка перед повторным запуском выявила ещё одну ошибку первоначальной DB-01: `ALTER DEFAULT PRIVILEGES ... IN SCHEMA ... REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC` не снимает глобальный стандартный EXECUTE PostgreSQL. Файл заменён на DB-01 v0.2: default privileges владельцев теперь отзывают PUBLIC глобально (без `IN SCHEMA`), а disposable probe-функции проверяют это фактически до `COMMIT`.
