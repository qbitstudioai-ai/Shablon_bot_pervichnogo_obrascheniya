...
| [~] DB-SCHEMA-01F | DB-SCHEMA-01 | `sql/DB-SCHEMA-01F_verify_renamed_schema.sql` v0.2: read-only verify новой schema, 25 tables и 28 SECURITY DEFINER functions по точным сигнатурам (26 обычных + ingress + attachment), owner/search_path/no old refs, EXECUTE 17/10/1, runtime DML denied, temporary DB CREATE revoked, production/canary present; v0.1 ошибочно считал только `(p_dannye jsonb)` и получил 26; после успеха Supabase-stage завершён и следующая сессия PRE-02/n8n |
