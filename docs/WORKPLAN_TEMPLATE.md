...
Текущий режим — только документация. Задачи реализации ниже описывают будущую последовательность и не означают разрешения менять сервер. Один ID — одна небольшая сессия; большой пункт сначала делится на подзадачи по [правилам документации](DOCUMENTATION_RULES.md).
...
| [ ] DOC-03 | В новой сессии заполнен черновик паспорта первой тестовой установки по доступным данным | Понятно, что известно и что предстоит выбрать; секретов нет, сервер не изменён |
...
|---|---|---|
| [ ] PRE-01 | DOC-03, разрешение на реализацию | Проверен паспорт сервера: версии n8n/Supabase/PostgreSQL/pgvector, Auth, подключения, ограничения каналов и гарантия надёжного приёма до подтверждения webhook; выбран реализуемый путь для первого канала |
| [ ] PRE-02 | PRE-01 | Зафиксирован профиль обработки: LLM, embedding-модель/размерность, tokenizer, Markdown/YAML-парсер, библиотеки, лимиты, top-k/порог и контрольный набор; подтверждена совместимость версий |
...
| Статус / ID | Зависимости | Один результат и критерий |
|---|---|---|
| [ ] DB-01 | PRE-01 | Создана и проверена основа двух тестовых schema с ограниченными ролями; чтение/запись чужой schema запрещены в обе стороны |
| [ ] DB-02 | DB-01 | Созданы сообщения, пользователи, диалоги и события; повтор входа не создаёт второго сообщения, связи и русские описания проверены |
| [ ] DB-03 | DB-02 | Созданы очередь, короткая память, исходящие действия и напоминания; версии/блокировки/повторы проверены |
| [ ] DB-04 | DB-01, PRE-02 | Созданы документы, версии, фрагменты и проверки знаний; ограничения допускают много архивных версий и один активный указатель |
| [ ] DB-05 | DB-04 | Проверены отдельный поиск по активным знаниям, проверочный поиск по черновику и атомарная публикация; клиентский доступ к черновику запрещён |
...
| Статус / ID | Зависимости | Один результат и критерий |
|---|---|---|
| [ ] KB-01 | RT-01, DB-04, PRE-02 | Собственный тестовый служебный Telegram Trigger принимает .md только от разрешённого Павла; неверный источник, размер и структура отклоняются |
...
| Статус / ID | Зависимости | Один результат и критерий |
|---|---|---|
| [ ] BOT-01 | RT-02, KB-03 | CORE отвечает в исходный канал с короткой памятью и активными знаниями своей компании; поля LLM проверяются, неподтверждённые факты не выдаются |
| [ ] BOT-02 | BOT-01 | Проверены этапы, цели, локальная заявка, завершение и передача человеку; фраза о принятой заявке не включает напоминания |
| [ ] BOT-03 | BOT-02 | Проверены сроки 20 минут / 24 часа / ещё 24 часа, отмена, задержки и одновременный ответ; технический сбой не записывается как молчание клиента |
