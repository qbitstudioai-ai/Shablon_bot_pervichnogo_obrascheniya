...
| [~] DB-03D2 | DB-03D1 | Подготовлен `sql/DB-03D2_take_return_manual.sql` v0.1: 4 новые SECURITY DEFINER функции + controlled `CREATE OR REPLACE` C4 claim/result без смены сигнатур; Take отменяет wait/reminders/internal jobs и только не начатые bot/system actions, manager outgoing идёт через общую client-bot очередь, foreign manager deny, Return не создаёт client action, private alert не принимает chat_id; server-check ещё не выполнен |
