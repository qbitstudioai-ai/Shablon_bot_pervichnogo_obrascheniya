...
| [~] PRE-02A | PRE-01 | Через отдельный Amsterdam n8n AI relay выполнен тестовый вызов OpenRouter `deepseek/deepseek-v4.1-flash` через test Credential; сохранены обезличенные вход/статус/модель/структура ответа, секрет не попал в workflow/GitHub. Российский n8n остаётся CORE/runtime и не обращается к OpenRouter напрямую, пока такой доступ не подтверждён |
| [ ] PRE-02B | PRE-02A | Через Amsterdam AI relay проверены оба обязательных embedding-вызова — для очищенного document chunk и для очищенного query — `qwen/qwen3-embedding-8b` с `dimensions=1024`; оба вектора имеют длину 1024, один профиль используется при индексации и поиске, либо документирован переход на резерв без смешивания профилей |
