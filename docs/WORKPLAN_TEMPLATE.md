...
| [ ] WF-02 | WF-01, DB-03, PRE-02A | Синхронизированы клиентский и служебный workflow с проверенным DB-03 JSONB API, разделены клиентские/служебные ветки, удалены instance-specific metadata и будущие DB-04/DB-05 вызовы из запускаемых путей; очищенные экспорты повторно импортируются в n8n 2.41.0 без ошибок и только после этого сохраняются в GitHub |
