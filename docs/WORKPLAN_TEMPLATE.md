...
| [~] DB-03C1 | DB-03B | Подготовлен `sql/DB-03C1_client_ingress.sql` v0.1: 4 SECURITY DEFINER API-функции, EXECUTE только `qbit_test_bot`, 2 UNIQUE-index для idempotent attachment retry; probe проверяет same-key/same-hash, different-hash conflict, duplicate external message даже после закрытия dialog, wait/reminder cancellation, attachment/STT/PII idempotency; на сервер ещё не применён |
