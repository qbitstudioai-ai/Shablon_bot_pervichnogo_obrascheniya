...
| [~] DB-02 | DB-01 | Подготовлен полный `sql/DB-02_dialog_content.sql` v0.1 для `qbit_test`: 13 таблиц, 182 русских COMMENT и 37 контрактных индексов; встроенный SAVEPOINT-probe проверяет FK/CHECK/UNIQUE и отклонение второго `vneshnee_soobshchenie_id` в одном диалоге, затем удаляет probe-данные откатом. На сервер ещё не применён; полная идемпотентность integration event остаётся DB-03 |
| [ ] DB-03 | DB-02 | Созданы очереди диалогов/исходящих/зеркала, память, guard/блокировка, напоминания и операторский Telegram; аренды/CAS, два одновременных «Забрать», ручной ответ, неизвестный результат и отмена позднего bot-ответа проверены |
