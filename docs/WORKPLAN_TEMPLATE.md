...
| [ ] DB-03V | DB-03D | Интегрально проверить критический переход: bot outgoing уже `v_rabote` → manager Take → поздний `podtverzhdeno`; внешний confirmed-факт должен сохраниться, но owner остаётся `chelovek`, stale effects/wait/reminders не восстанавливаются; после проверки можно закрыть родительский DB-03 |
