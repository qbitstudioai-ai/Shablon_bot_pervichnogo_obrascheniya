...
| [~] DB-03A | DB-02 | Подготовлен `sql/DB-03A_reliability_core.sql` v0.1: 9 core-таблиц, 129 COMMENT на поля, 25 контрактных индексов, forward FK `soobshcheniya.sobytie_id → sobytiya_integraciy.id`, default-deny и SAVEPOINT-probe уникальности/lease/outgoing/reminder; на сервер ещё не применён |
