...
| [~] DB-01 | DB-00, PRE-01 | Подготовлен полный `sql/DB-01_test_schemas_roles.sql`: две test schema `qbit_test` и `kompaniya_001_test`, 12 ограниченных ролей, default-deny/default privileges и встроенные проверки двусторонней изоляции. На сервер ещё не применён; задача закрывается только после успешного запуска и чтения доказательств |
| [ ] DB-02 | DB-01 | Созданы пользователи, идентификаторы каналов, сообщения, диалоги, вложения, транскрипции, факты/PII и события; повтор входа не создаёт второго сообщения, связи и русские COMMENT проверены |
