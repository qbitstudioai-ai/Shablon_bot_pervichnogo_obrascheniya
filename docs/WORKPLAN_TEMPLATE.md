...
| [~] DB-03C1 | DB-03B | v0.1 (42702 ambiguity) и v0.2 (42703 topic variable mismatch) остановились до COMMIT и откатились; подготовлен `sql/DB-03C1_client_ingress.sql` v0.3: 4 SECURITY DEFINER API-функции, исправленные topic targets, статический audit без undeclared `v_*`, EXECUTE только `qbit_test_bot`, 2 UNIQUE-index; probes сохраняют idempotency/conflict/closed-dialog/wait/STT/PII и RU phone normalization; v0.3 ещё не применён |
