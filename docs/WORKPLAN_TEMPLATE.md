...
| [~] DB-03C4 | DB-03C3 | Подготовлен `sql/DB-03C4_outgoing_reminders.sql` v0.2: 5 SECURITY DEFINER функций; stable outgoing key сравнивает содержимое, claim/reclaim использует lease/fencing, unknown не повторяется вслепую, confirmed-факт сохраняется даже при in-flight изменении dialog, но stale dependent effects подавляются; t0 только от confirmed main reply, reminders не двигают t0, confirmed reminder2 создаёт durable loss-check; probe проверяет unknown/retry/confirmed, stale-confirm crossing input, mirror main+reminders, reminder1/2 и net_otveta; v0.1 остановился до COMMIT из-за неверного hardcoded generation в probe, v0.2 использует фактическую generation/version; на сервер ещё не применён |
