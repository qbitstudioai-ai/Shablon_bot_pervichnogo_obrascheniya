...
| [~] DB-03C1 | DB-03B | v0.1 server-run остановился до COMMIT на PL/pgSQL ambiguity `versiya_dialoga` и откатился; подготовлен `sql/DB-03C1_client_ingress.sql` v0.2: 4 SECURITY DEFINER API-функции, `#variable_conflict use_column`, явная квалификация критических RETURNING/UPDATE, EXECUTE только `qbit_test_bot`, 2 UNIQUE-index; probe сохраняет проверки idempotency/conflict/closed-dialog/wait/STT/PII и проверяет RU phone normalization для `8(...)`, digits-only и `+7 ...`; v0.2 ещё не применён |
| [ ] DB-03C2 | DB-03C1 | Реализованы `poluchit_kontekst_dialoga`, `sohranit_fakty_i_pamyat`, `proverit_limit_chastoty`, `zapisat_narushenie_tematiky`, `razblokirovat_polzovatelya`; PII не смешивается с AI context, CAS памяти/диалога и блокировка по policy проверены |
