...
| [~] DB-03V | DB-03D | Подготовлен `sql/DB-03V_late_confirm_after_take.sql` v0.1 без permanent DDL: disposable SAVEPOINT probe моделирует конкретный bot action в точном C4 `v_rabote`/lease/fencing state, затем manager Take и late confirmed; проверяет внешний факт/message ID, human owner/version, отсутствие stale stage/goal/wait/t0/reminders и mirror `effekty_primeneny=false`; все probe rows rollback; server-check ещё не выполнен |
