...
| [~] DB-SCHEMA-01 | DB-03 | `sql/DB-SCHEMA-01_rename_qbit_schema.sql` v0.3: переименовать `qbit_test` → `qbit_bot_pervichnogo_obrascheniya`, обновить fixed search_path/qualified function bodies, сохранить роли/ACL; v0.1 остановился из-за отсутствия database CREATE; v0.2 — из-за SELECT на TEMP snapshot после SET ROLE; v0.3 временно grant database CREATE только для owner rename/replace, затем до post-assertions делает RESET ROLE→postgres и REVOKE CREATE; snapshots читаются trusted session role без дополнительных grants; проверяет отсутствие старой schema, сохранность 25 tables/28 functions и неизменность production/canary; после server-check можно переходить к PRE-02/n8n |
