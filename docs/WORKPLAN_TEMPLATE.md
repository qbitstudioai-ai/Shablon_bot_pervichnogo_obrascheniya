...
| [~] DB-SCHEMA-01 | DB-03 | Rename фактически уже произошёл: последующий запуск сообщает, что `qbit_test` отсутствует; v0.3 имел ошибочный post-COMMIT reporting SELECT. Старую rename-миграцию больше не запускать. Родитель закрывается после read-only DB-SCHEMA-01F |
| [~] DB-SCHEMA-01F | DB-SCHEMA-01 | `sql/DB-SCHEMA-01F_verify_renamed_schema.sql` v0.3: read-only verify новой schema, 25 tables и 28 SECURITY DEFINER functions по точным сигнатурам (26 обычных + ingress + attachment), owner/search_path/no old refs, EXECUTE 17/10/1, runtime DML denied, temporary DB CREATE revoked, canary present; production `qbit` не требуется в test-stage и только сообщается информационно. v0.1 ошибочно считал только `(p_dannye jsonb)` и получил 26; v0.2 ошибочно требовал наличие production `qbit`; после успеха Supabase-stage завершён и следующая сессия PRE-02/n8n |
