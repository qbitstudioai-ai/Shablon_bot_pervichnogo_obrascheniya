...
| [~] DB-03D | DB-03B, DB-03C | Составной операторский этап; закрывается после DB-03D1 и DB-03D2 и интегральной проверки конкурентного Take/Return/manual outgoing/private alert |
| [~] DB-03D1 | DB-03B, DB-03C | Подготовлен `sql/DB-03D1_service_topic_mirror.sql` v0.1: 7 SECURITY DEFINER функций для durable service ingress, private chat allowlist, topic claim/confirm/unknown и mirror lease/fencing; probe проверяет service idempotency/conflict, unknown manager deny, topic first-owner/confirmed/unknown no-blind-recreate, narrow client text/media/private target, retry→unknown terminal и отсутствие общего raw SELECT; на сервер ещё не применён |
| [ ] DB-03D2 | DB-03D1 | Реализовать `zabrat_dialog_operatorom`, `vernut_dialog_botu`, `sozdat_ruchnoe_ishodyashchee`, `sozdat_lichnoe_uvedomlenie`; проверить first-commit-wins Take, чужой manager deny, Return без автоответа и private alert trusted target |
