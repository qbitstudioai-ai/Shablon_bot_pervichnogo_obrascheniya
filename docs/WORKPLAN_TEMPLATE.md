...
| [~] DB-SCHEMA-01F | DB-SCHEMA-01 | `sql/DB-SCHEMA-01F_v0.4_verify_renamed_schema.sql` v0.4 FRESH-PATH: тот же read-only verify новой schema, 25 tables и 28 SECURITY DEFINER functions по точным сигнатурам, owner/search_path/no old refs, EXECUTE 17/10/1, runtime DML denied, temporary DB CREATE revoked, canary present; production `qbit` не требуется. Новый filename исключает повторное выполнение stale v0.2; итог содержит `verifier_version=DB-SCHEMA-01F_v0.4_fresh_path`. После успеха Supabase-stage завершён и следующая сессия PRE-02/n8n |
