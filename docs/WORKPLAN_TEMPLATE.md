...
| [~] DB-03C3 | DB-03C2 | Подготовлен `sql/DB-03C3_processing_queue.sql` v0.1: 3 SECURITY DEFINER функции; claim через job+dialog `FOR UPDATE ... SKIP LOCKED`, unique one-active-job backstop, monotonic fencing number, expired lease reclaim, heartbeat не продлевает stale dialog/state, completion требует live lease+worker+fencing+dialog version; probe проверяет one-active dialog, stale heartbeat/completion, stale cleanup, reclaim, old-worker fencing, retry/reclaim, success/error/cancel; на сервер ещё не применён |
