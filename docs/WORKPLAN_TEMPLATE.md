...
| [~] DB-03B | DB-03A | Подготовлен `sql/DB-03B_operator_tables.sql` v0.1: 3 операторские таблицы, 54 COMMENT на поля, 10 контрактных индексов, FK `dialogi.tekushchiy_menedzher_id → menedzhery_telegram.id`, default-deny и SAVEPOINT-probe manager/private-chat/topic/mirror ограничений; на сервер ещё не применён |
