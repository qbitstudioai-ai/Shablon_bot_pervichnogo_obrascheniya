...
| [ ] DB-03A | DB-02 | Созданы core-таблицы надёжности: `sobytiya_integraciy`, `zadaniya_obrabotki`, `ishodyashchie_deystviya`, `napominaniya`, `pamyat_dialoga`, `analiz_dialogov`, `obratnaya_svyaz`, `sistemnye_sobytiya`, `zhurnal_administrirovaniya`; добавлен FK `soobshcheniya.sobytie_id`; индексы/lease-ограничения/COMMENT проверены |
| [ ] DB-03B | DB-03A | Созданы `menedzhery_telegram`, `operator_telegram_temy`, `sobytiya_zerkala_operatora`; добавлен FK `dialogi.tekushchiy_menedzher_id`; topic/mirror/manager ограничения и default-deny проверены |
| [ ] DB-03C | DB-03A, DB-03B | Реализованы клиентский ingress, guard, память, очередь, исходящие и напоминания как узкие PostgreSQL-функции; идемпотентность same-key/same-hash, conflict по другому hash, lease/CAS/stale version и `neizvestno` проверены |
